...
proptest! {
    ...
    // ---------- OB-LQ-P-03: DimensionScore::passes is monotone in score ----------

    #[test]
    fn prop_dimension_passes_monotone_in_score(
        d in arb_quality_dimension(),
        s_lo in 0u8..=100,
        s_hi in 0u8..=100,
        threshold in 0u8..=100,
    ) {
        prop_assume!(s_hi >= s_lo);
        let lo = DimensionScore::new(d, s_lo).unwrap();
        let hi = DimensionScore::new(d, s_hi).unwrap();
        // If hi passes threshold, lo must pass it too.
        if hi.passes(threshold) {
            prop_assert!(
                lo.passes(threshold),
                "monotonicity violated: lo={:?} hi={:?} threshold={:?}",
                s_lo, s_hi, threshold
            );
        }
    }
    ...
    // ---------- OB-LQ-P-06: QualityScore::passes is monotone in overall ----------

    #[test]
    fn prop_quality_score_passes_monotone(
        overall_lo in 0u8..=100,
        overall_hi in 0u8..=100,
        threshold in 0u8..=100,
        dims in proptest::collection::vec(arb_dimension_score(), 0..=5),
        issues in proptest::collection::vec(arb_quality_issue(), 0..=3),
    ) {
        prop_assume!(overall_hi >= overall_lo);
        let lo = QualityScore::new(overall_lo, dims.clone(), issues.clone()).unwrap();
        let hi = QualityScore::new(overall_hi, dims, issues).unwrap();
        if hi.passes(threshold) {
            prop_assert!(
                lo.passes(threshold),
                "monotonicity violated: lo_overall={:?} hi_overall={:?} threshold={:?}",
                overall_lo, overall_hi, threshold
            );
        }
    }

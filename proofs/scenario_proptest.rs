...
use clarity_web::domain::scenario::{
    Hole, HolePunchingResults, HoleType, ScenarioField,
};
...
fn arb_scenario_field() -> impl Strategy<Value = ScenarioField> {
    ...
    ).prop_map(|(trigger, value_moment, feeling, hole_punching)| {
        ScenarioField {
            trigger,
            value_moment,
            feeling,
...
/// PO-SC-P-03 part 3: addressed_count is monotonic non-decreasing (Law L3).
#[test]
fn proptest_hole_punching_monotonic() {
    proptest!(|(r in arb_hole_punching_results(), ht in arb_hole_type(), expl in ".*")| {
        let before = r.addressed_count();
        let after = r.address(ht, expl).addressed_count();
        prop_assert!(after >= before, "addressed_count must not decrease (L3 monotonic); before={}, after={}", before, after);
    });
}
...
fn proptest_with_severity_clamp() {
    ...
    for (input, expected) in boundary_cases {
        ...
        assert_eq!(hole.severity, expected,

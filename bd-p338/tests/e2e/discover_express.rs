...
impl MockExtractionProvider {
    ...
    fn extract_from_text(&self, text: &str) -> clarity_web::providers::ExtractedFields {
        let word_count = text.split_whitespace().count();
        let confidence = if word_count > 50 { 0.85 } else if word_count > 20 { 0.65 } else { 0.45 };

        use clarity_web::providers::{ExtractionMetadata, FieldExtraction, FieldType};
        use serde_json::json;
...
impl clarity_web::providers::ExtractionProvider for MockExtractionProvider {
    async fn extract_fields(
        &self,
        text: &str,
        _context: &clarity_web::providers::ExtractionContext,
    ) -> Result<clarity_web::providers::ExtractedFields, clarity_web::providers::ExtractionError> {
        if self.delay_ms > 0 {
            tokio::time::sleep(std::time::Duration::from_millis(self.delay_ms)).await;
        }

        if text.trim().is_empty() {
            return Err(clarity_web::providers::ExtractionError::InvalidInput(
    ...
    async fn extract_fields_with_schema(
        ...
        context: &clarity_web::providers::ExtractionContext,
    ) -> Result<clarity_web::providers::ExtractedFields, clarity_web::providers::ExtractionError> {
    ...
    async fn health_check(&self) -> Result<(), clarity_web::providers::ExtractionError> {
        Ok(())
    }
}
...
mod tests {
    ...
    use clarity_web::providers::{ExtractionContext, ExtractionError, ExtractionProvider, ExtractedFields, FieldType, SchemaField};
    ...
    fn test_field_initialization() {
        ...
        for (i, field) in fields.iter().enumerate() {
            assert_eq!(field.id, expected_ids[i]);
        }
    }
    ...
    fn test_minimum_extraction_characters() {
        ...
        assert_eq!(MIN_EXTRACTION_CHARS, MIN_CHARS);

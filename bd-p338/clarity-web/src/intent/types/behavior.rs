...
pub struct Behavior {
  ...
  #[serde(default)]
  pub notes: String,
  /// Behavior dependencies (behavior references)
  #[serde(default)]
  pub requires: Vec<String>,
  /// Classification tags
  #[serde(default)]
  pub tags: Vec<String>,
  /// How to verify this behavior works
  #[serde(default)]
...
impl Behavior {
  ...
  /// Builder method to set requires
  #[must_use]
  pub fn with_requires(self, requires: Vec<String>) -> Self {
    Self { requires, ..self }
  }
...
mod tests {
  ...
  fn test_serde_roundtrip_behavior() {
    let behavior = match Behavior::new("login".to_string()) {
      Ok(value) => value.with_intent("User login".to_string()),
      Err(_) => return,
    };

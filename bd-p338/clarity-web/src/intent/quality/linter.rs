...
impl SpecLinter {
  ...
  fn check_deprecated_patterns(spec: &Spec, report: &mut LintReport) {
    ...
    for (idx, feature) in spec.features.iter().enumerate() {
      for (bidx, behavior) in feature.behaviors.iter().enumerate() {
        let desc_lower = behavior.intent.to_lowercase();
          ...
          if desc_lower.contains(term) {
            report.add_result(
              LintResult::new(
                ...
                severity,
                format!("features[{idx}].behaviors[{bidx}].intent"),
                format!("Behavior intent contains vague term '{term}'",),
              )
  ...
  fn check_description_quality(spec: &Spec, report: &mut LintReport) {
    ...
    for (idx, feature) in spec.features.iter().enumerate() {
      ...
      for (bidx, behavior) in feature.behaviors.iter().enumerate() {
        if behavior.intent.trim().is_empty() {
          report.add_result(
            LintResult::new(
              LintRule::DescriptionQuality,
              severity,
              format!("features[{idx}].behaviors[{bidx}].intent"),
              format!("Behavior '{}' has no intent", behavior.name),
            )
            .with_suggestion("Add an intent explaining what the behavior does".to_string()),
          );
        } else if behavior.intent.trim().len() < MIN_BEHAVIOR_DESCRIPTION_LENGTH {
          report.add_result(
            LintResult::new(
              LintRule::DescriptionQuality,
              LintSeverity::Hint,
              format!("features[{idx}].behaviors[{bidx}].intent"),
              format!("Behavior '{}' intent is very short", behavior.name),
            )
            .with_suggestion("Consider expanding the intent".to_string()),
          );
        }
      }
    }
    ...
    match Spec::new("test-spec".to_string()) {
      Ok(mut spec) => {
        ...
        let logout = match Behavior::new("logout".to_string()) {
          Ok(b) => b.with_intent("User logs out".to_string()),
          Err(_) => return spec,
        };
  ...
  fn create_problematic_spec() -> Spec {
    match Spec::new("problematic-spec".to_string()) {
      Ok(mut spec) => {
        ...
        vague_behavior.intent = "TODO: Implement this".to_string();

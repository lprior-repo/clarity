...
fn generate_implementation_hints(spec: &Spec) -> String {
  ...
  let suggested_stack = if hints.implementation.suggested_stack.is_empty() {
    String::new()
  } else {
    let stack_list = hints.implementation.suggested_stack.iter().map(|s| format!("- {s}")).join("\n");
    format!("### Suggested Stack\n\n{stack_list}\n\n")
  };

  let entities = generate_entity_hints(&hints.entities);
  let pitfalls = generate_pitfalls(&hints.pitfalls);

  format!(
    "## Implementation Hints\n\n{suggested_stack}{entities}{pitfalls}"
  )
}

fn generate_entity_hints(entities: &std::collections::HashMap<String, crate::intent::types::EntityHint>) -> String {
  ...
  let entity_list = entities
    ...
    .map(|(name, entity)| {
      let fields = if entity.fields.is_empty() {
        ...
        let field_list = entity.fields.iter().map(|(k, v)| format!("- {k}: {v}")).join("\n");
        format!("\n\n**Fields:**\n{field_list}")
      };
...
fn generate_pitfalls(pitfalls: &[String]) -> String {
  if pitfalls.is_empty() {
    return String::new();
  }

  let pitfall_list = pitfalls.iter().map(|p| format!("- {p}")).join("\n");
...
mod tests {
  ...
  use crate::intent::types::{
    AIHints, AntiPattern, Behavior, EntityHint, Feature, ImplementationHints, Invariant, SecurityHints,
    Verification,
  };
    ...
    Spec {
      ...
      ai_hints: AIHints {
        ...
        security: SecurityHints {
          password_hashing: "argon2".to_string(),
          jwt_algorithm: "HS256".to_string(),
          jwt_expiry: "1h".to_string(),
          rate_limiting: "100/min".to_string(),
        },
        pitfalls: vec!["Don't store passwords in plain text".to_string()],
      },
    }
  }
  ...
  fn test_generate_ready_document_includes_implementation_hints() {
    ...
    assert!(doc.contains("### Suggested Stack"));

...
pub fn calculate_ai_readiness_score(spec: &Spec) -> u8 {
  ...
  let has_impl_hints = !ai_hints.implementation.suggested_stack.is_empty();
  ...
  let has_pitfalls = !ai_hints.pitfalls.is_empty();
  let example_ratio = calculate_example_ratio(spec);
  ...
  let score = 100_u16
    ...
    .saturating_sub(u16::from(!has_pitfalls) * 5)
    .saturating_sub(u16::from(example_ratio < 0.5) * 15)

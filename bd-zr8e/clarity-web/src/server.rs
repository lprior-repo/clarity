...
#[cfg(not(target_arch = "wasm32"))]
use crate::providers::{resolve_from_provider_config, ResolvedProviderConfig};
#[cfg(feature = "server")]
...
/// Uses the shared `ResolvedProviderConfig` for deterministic resolution.
#[cfg(not(target_arch = "wasm32"))]
#[allow(dead_code)]
static AI_PROVIDER: LazyLock<Arc<OpenCodeProvider>> = LazyLock::new(|| {
  // Use shared resolution routine
  let resolved = load_ai_config()
    .map(|config| resolve_from_provider_config(&config.provider))
    .unwrap_or_else(|_| ResolvedProviderConfig::default());
  ...
  match OpenCodeProvider::new_with_options(
    ...
    OpenCodeProviderOptions {
      ...
      routing_provider: resolved.routing_provider.clone(),
    },
  ) {
    Ok(provider) => {
      tracing::info!(
    ...
    Err(error) => {
      tracing_warn!(error = %error, "Failed to create OpenCode provider, using fallback");
      // Fallback to known-working defaults
      match OpenCodeProvider::new_with_options(
        ...
        Err(fallback_error) => {
          tracing_warn!(error = %fallback_error, "Fallback OpenCode provider initialization failed");
          std::process::abort();
        }
      }
    }
...
fn resolve_provider_with_model_override(
  ...
  match model_override {
    None => Ok(Arc::clone(&AI_PROVIDER)),
    Some(model) => {
      // Parse model override: either "provider_id/model_id" or just "model_id"
      let (routing_provider, model_id) = model
        .split_once('/')
        .map(|(p, m)| (Some(p.to_string()), m.to_string()))
        .unwrap_or_else(|| (AI_PROVIDER.routing_provider().clone(), model.clone()));
...
///
/// This struct is guaranteed to never contain:
...
pub async fn get_ai_provider_status_server() -> Result<AiProviderDiagnostics, ServerFnError> {
  // Use shared resolution routine for deterministic diagnostics
  let resolved = load_ai_config()
    .map(|config| resolve_from_provider_config(&config.provider))
    .unwrap_or_else(|_| ResolvedProviderConfig::default());
...
pub async fn extract_fields_server(
  ...
  let provider = resolve_provider_with_model_override(&model_override)?;

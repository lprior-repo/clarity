...
impl Session {
    ...
    /// let updated = session.transition_to(SessionState::InProgress, Timestamp::now());
    /// assert!(updated.is_ok());
    ///
    /// // Invalid transition: Completed -> InProgress
    /// let completed = updated.unwrap().transition_to(SessionState::Completed, Timestamp::now()).unwrap();
    /// let invalid = completed.transition_to(SessionState::InProgress, Timestamp::now());
...
mod tests {
    ...
    fn test_session_new() {
        ...
        let created_at = Timestamp::from_secs(1234567890);
    ...
    fn test_session_transition_to_in_progress() {
        ...
        let updated = session
            .transition_to(SessionState::InProgress, Timestamp::from_secs(1234567891))
            .unwrap();

        assert_eq!(updated.state, SessionState::InProgress);
        assert_eq!(updated.updated_at.as_secs(), 1234567891);
    ...
    fn test_session_transition_to_cancelled() {
        ...
        let cancelled = session
            .transition_to(SessionState::Cancelled, Timestamp::from_secs(1234567891))
            .unwrap();
    ...
    fn test_timestamp_from_secs() {
        let ts = Timestamp::from_secs(1234567890);

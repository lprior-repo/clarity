...
use dioxus::prelude::*;
use crate::settings::{Settings, Theme, BackupFrequency, validate_beads_per_page, validate_data_location};
    ...
    // Load settings on mount
    {
        ...
        use_effect(move || {
            ...
            match Settings::load() {
                ...
                Err(e) => {
                    *settings_state_clone.write() = SettingsState::with_error(
...
pub fn use_data_location_validator() -> Callback<PathBuf, Option<String>> {
    Callback::new(|path: PathBuf| {
        validate_data_location(&path).err().map(|e| e.to_string())
    })
}

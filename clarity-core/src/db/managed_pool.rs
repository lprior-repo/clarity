...
//!
//! This module provides advanced connection pooling using deadpool-sqlx:
...
use deadpool_sqlx::{Manager, Pool, Runtime};
...
/// Managed pool wrapper for deadpool-sqlx
...
impl ManagedPool {
  ...
  async fn create_sqlite_pool(config: &ManagedPoolConfig) -> DbResult<Self> {
    let manager = Manager::from_config(
      &config.database_url,
      deadpool_sqlx::sqlite::Config::default(),
      Runtime::Tokio1,
    );
  ...
  async fn create_postgres_pool(config: &ManagedPoolConfig) -> DbResult<Self> {
    let manager = Manager::from_config(
      &config.database_url,
      deadpool_sqlx::postgres::Config::default(),
      Runtime::Tokio1,
    );

...
impl SchemaRegistry {
  ...
  pub fn register(&mut self, schema: Schema) -> Result<(), SchemaRegistryError> {
    ...
    // Use references for lookup to avoid clones
    let key_ref = (&schema.id, &schema.version);
    if schemas.contains_key(&key_ref) {
      return Err(SchemaRegistryError::DuplicateSchema {
        id: schema.id.as_str().to_string(),
        version: schema.version.as_str().to_string(),
      });
    }

    // Move into the HashMap key (no clone needed)
    schemas.insert((schema.id, schema.version), schema);
    Ok(())
  }

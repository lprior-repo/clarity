    ...
    pub fn new(
        ...
        let status_distribution = ProgressDistribution {
            ...
            deferred_pct: if total > 0 {
                (deferred as f64 / total as f64) * 100.0
            } else {
                0.0
            },
        ...
        let counts = statuses.iter().fold(
            ...
            |(completed, in_progress, blocked, deferred, not_started), status| match status {
                ProgressStatus::Completed => {
                    (completed + 1, in_progress, blocked, deferred, not_started)
                }
                ProgressStatus::InProgress => {
                    (completed, in_progress + 1, blocked, deferred, not_started)
                }
                ProgressStatus::Blocked => {
                    (completed, in_progress, blocked + 1, deferred, not_started)
                }
                ProgressStatus::Deferred => {
                    (completed, in_progress, blocked, deferred + 1, not_started)
                }
                ProgressStatus::NotStarted => {
                    (completed, in_progress, blocked, deferred, not_started + 1)
                }
            },
        );
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::InvalidCount { total, sum } => {
                write!(f, "invalid count values: total={}, sum={}", total, sum)
            }
        }
    }
}
...
/// ```
pub fn format_json_progress(metrics: &ProgressMetrics) -> String {
...
pub fn format_progress(metrics: &ProgressMetrics, options: ProgressOutputOptions) -> String {
    match options.format {
        ProgressFormat::Terminal => format_terminal_progress(metrics),
        ProgressFormat::Json => format_json_progress(metrics),
        ProgressFormat::Markdown => format_markdown_progress(metrics),
    }
}
...
mod tests {
    ...
    fn test_format_progress_terminal() {
        let metrics = ProgressMetrics::new(10, 7, 2, 0, 1, 0).unwrap();
        let output = format_progress(&metrics, ProgressOutputOptions::default());
        assert!(output.contains("["));
        assert!(output.contains("]"));
    ...
    fn test_format_progress_json() {
        ...
        let output = format_progress(&metrics, options);

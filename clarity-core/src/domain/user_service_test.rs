...
//! User service tests (isolated from database)
...
use super::core::DomainError;
use crate::auth::{AuthError, hash_password, verify_password, validate_password_strength, AuthResult};
...
fn test_create_user_success() {
    ...
    let (new_state, user) = create_user(state, email.clone(), password, role).unwrap();
...
fn test_update_user_password() {
    ...
    let (new_state, updated_user) = update_user_password(
        new_state,
        user.id,
        current_password,
        new_password,
...
fn test_get_users_by_role() {
    ...
    // Create admin user
    let (state, _) = create_user(state.clone(), email1.clone(), password.clone(), UserRole::Admin).unwrap();
...
fn test_user_pipeline_operations() {
    ...
    let admin_users = process_user_pipeline(
        all_users,
        &[filter_admin_users],
    );

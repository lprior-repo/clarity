...
pub const fn map_db_error(error: &DbError) -> Result<ExitCode, ExitCodeError> {
  match error {
    ...
    | DbError::BundledDbExtraction(_)
    | DbError::BundledDbConnection(_)
    | DbError::PoolExhausted(_)
    | DbError::PoolClosed
    | DbError::PoolCreation(_)
    | DbError::AcquisitionTimeout(_)
    | DbError::ReconnectionFailed(_) => Ok(ExitCode::IO_ERROR),
...
///
/// Returns `ExitCodeError::OutOfRange` if the mapped exit code is > 255
pub const fn map_validation_error(error: &ValidationError) -> Result<ExitCode, ExitCodeError> {
  match error {

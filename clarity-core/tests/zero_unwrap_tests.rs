...
#[test]
fn test_result_with_expected_error() {
  let result: Result<i32, String> = Err("error".to_string());
  assert!(result.is_err());
}

#[test]
fn test_result_with_ok_value() {
...
#[test]
fn test_result_or_else_pattern() {
  let result: Result<i32, String> = Ok(42);
...
#[test]
fn test_result_and_then_pattern() {
  let result: Result<i32, String> = Ok(42);
  let value = result.and_then(|x| Ok(x + 1));
  if let Ok(value) = value {
    assert_eq!(value, 43);
  }
}

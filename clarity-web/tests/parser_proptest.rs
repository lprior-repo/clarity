...
use clarity_web::intent::types::{AIHints, AntiPattern, Behavior, Feature, Invariant, Spec};
...
fn arb_behavior() -> impl Strategy<Value = Behavior> {
  (
    ...
    .prop_map(
      |(name, description, verification_type, pre, post)| Behavior {
        name,
        description,
...
fn arb_invariant() -> impl Strategy<Value = Invariant> {
  (arb_invariant_name(), arb_opt_string(), arb_opt_string()).prop_map(
    |(name, description, constraint)| Invariant {
      name,
      description,
      constraint,
    },
...
fn arb_spec() -> impl Strategy<Value = Spec> {
  (
    ...
    .prop_map(
      |(name, description, features, invariants, anti_patterns, ai_hints)| Spec {
        name,
        description,
        features,
        invariants,
...
fn arb_non_object_value() -> impl Strategy<Value = Value> {
  prop_oneof![
    ...
    proptest::collection::vec(arb_opt_string().prop_map(Value::String), 0..4)
...
proptest! {
  ...
  fn proptest_p1_roundtrip(spec in arb_spec()) {
    ...
    // Use `&` to borrow — `prop_assert_eq!(x, y)` consumes `x` and `y`,
    // which would partially move `parsed.name` and `spec.name` out of
    // the still-needed whole-Spec values below.
...
proptest! {
  ...
  fn proptest_p8_from_value_matches_parse_spec(spec in arb_spec()) {
    ...
    // (b) `from_value == spec` — strongest property: `parse_spec_from_value`
    //     recovers the exact original Spec. Borrow both sides so neither is
    //     consumed by `prop_assert_eq!` (which moves its arguments); the
    //     second assertion below still needs `from_value` intact.
    prop_assert_eq!(&from_value, &spec);
    // (a) `from_value == from_string` — divergence check between the two
    //     parser entry points. Catches any path divergence independent
    //     of the original Spec's contents.

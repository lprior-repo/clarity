...
impl ExtractionProvider for MockExtractionProvider {
  async fn extract_fields(
    ...
    Ok(ExtractedFields { fields, metadata })
  }
...
impl ModeStateMachine {
  ...
  async fn extract_fields(
    ...
    let extracted = provider
      .extract_fields(&self.express_content, &context)
      .await?;
  ...
  fn verify_no_data_loss(&self) -> bool {
    ...
    let guided_step_ids: std::collections::HashSet<_> = self
      .guided_answers
      .iter()
...
async fn test_mode_preference_survives_reload() {
  // Setup with storage
  let storage = Arc::new(RedbStore::open_in_memory().expect("Failed to create in-memory store"));
  let provider = MockExtractionProvider::new();

  // First session: Set mode to Guided
  {
    let mut state_machine = ModeStateMachine::new().with_storage((*storage).clone());
  ...
  let reloaded_metadata = storage.get_metadata().expect("Should read metadata");
  assert!(
...
async fn test_partial_data_guided_to_express() {
  ...
  state_machine.answer_guided_question("solution", "Improve onboarding flow".to_string());
...
async fn test_full_e2e_mode_switch_workflow() {
  ...
  assert!(
    problem_card.content.contains("time zone") || problem_card.content.contains("remote"),

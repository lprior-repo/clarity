...
use clarity_web::types::{
    get_steps_for_phase, prompt_steps, Answer, PHASES,
};
use clarity_web::components::discover::{
    GuidedFlow, QuestionState, SuggestionProvider,
};
use clarity_web::components::discover::mode_toggle::DiscoverMode;
use clarity_web::lattice::quality::{
    calculate_quality, QualityScore, InversionControl,
};
...
impl TestState {
    ...
    fn update_quality_score(&mut self) {
        ...
        let quality_answers = self
            .answers
            .iter()
            .map(|a| clarity_web::lattice::quality::Answer {
...
async fn test_discover_guided_complete_flow() {
    ...
    // Step 1: Verify app loads in Discover phase
    {
        ...
        assert_eq!(s.current_phase, "discover", "App should start in Discover phase");
        assert_eq!(s.mode, DiscoverMode::Guided, "Guided mode should be default");
    }

    // Step 2: Verify Discover phase has 5 questions
    let discover_steps = get_steps_for_phase("discover");
    assert_eq!(discover_steps.len(), 5, "Discover phase should have 5 questions");
    ...
    assert!(!suggestion.is_empty(), "AI suggest should return non-empty suggestion");
    assert!(
        suggestion.len() > 50,
    ...
    // Verify progress updated
    {
        ...
        assert!(s.quality_score.is_some(), "Quality score should be calculated");
        assert!(s.quality_score.unwrap_or(0) > 0, "Quality score should be positive");
        assert!(
    ...
    {
        let s = state.lock().unwrap();
        assert_eq!(s.progress_text, "5/5 answered", "All 5 questions should be answered");
        assert_eq!(s.answers.len(), 5, "Should have 5 answers");
        assert!(
            s.discover_complete(),
            "Discover phase should be complete"
        );
        assert_eq!(
    ...
    // Step 12: Verify follow-up questions are displayed
    {
        ...
        for step_id in &["problem", "antithesis", "solution", "persona", "scenario"] {
            assert!(
                ...
                "Follow-up for {} should be visible",
                step_id
            );
        }
    }
    ...
    {
        let mut s = state.lock().unwrap();
        assert!(s.discover_complete(), "Discover must be complete to continue");
        // In the real UI, the continue button appears when phase is complete
        s.continue_visible = true;
        assert!(s.continue_visible, "Continue to Define button should be visible");
    }
    ...
    // Step 14: Verify phase transition
    {
        ...
        assert_eq!(s.current_phase, "define", "Should transition to Define phase");
    }
...
async fn test_ai_suggest_for_all_questions() {
    ...
    for step_id in question_ids {
        ...
        assert!(
            suggestion.len() > 30,
            "Suggestion for {} should be meaningful (at least 30 chars)",
            step_id
        );
...
async fn test_follow_up_display() {
    ...
    for step in discover_steps.iter() {
        assert!(
...
fn test_question_state_management() {
    ...
    for step in get_steps_for_phase("discover").iter() {
...
fn test_mock_suggestions_quality() {
    ...
    for step_id in step_ids {
        ...
        println!("✓ Mock suggestion for '{}' is realistic ({} chars)", step_id, suggestion.len());
    }
}
...
async fn test_complete_guided_flow_integration() {
    ...
    let first_question = get_steps_for_phase("discover").first().unwrap();
    assert_eq!(first_question.id, "problem");
    println!("✓ Phase 2: First question displayed: '{}'", first_question.title);

    // Phase 3: Get AI suggestion
    let suggestion = get_mock_suggestion("problem");
    assert!(!suggestion.is_empty());
    println!("✓ Phase 3: AI suggest provides suggestion ({} chars)", suggestion.len());
    ...
    for (i, (step_id, value)) in answers_data.iter().enumerate() {
        ...
        println!("✓ Phase {}: Answer submitted for '{}' - Progress: {}",
                 4 + i, step_id, progress);
    ...
    // Verify quality score
    {
        ...
        println!("✓ Phase 11: Quality score visible: {}", s.quality_score.unwrap());
    }

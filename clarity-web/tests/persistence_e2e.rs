...
fn count_table_records(db_path: &Path, table_name: &str) -> Result<usize, String> {
    ...
    match txn.open_table(table_def) {
        Ok(table) => {
            let mut count = 0;
            let iter = table.iter().map_err(|e| format!("Failed to iterate table: {}", e))?;
            for _ in iter {
                count += 1;
            }
            Ok(count)
...
fn test_e2e_persistence_discover_phase_express_mode() {
    ...
    let metadata = ProjectMetadata::with_current_timestamp("express".to_string(), "discover".to_string());
    store
    ...
    let answer_count = count_table_records(&db_path, tables::ANSWERS).expect("Failed to count answers");
    assert_eq!(
        answer_count, 5,
    ...
    let metadata_count =
        count_table_records(&db_path, tables::PROJECT_METADATA).expect("Failed to count metadata records");
    assert_eq!(
        metadata_count, 1,
    ...
    let lattice_count =
        count_table_records(&db_path, tables::LATTICE_CACHE).expect("Failed to count lattice cache records");
    assert_eq!(
        lattice_count, 1,
    ...
    let loaded_lattice = store.get_lattice_cache("discover").expect("Failed to get lattice cache");
...
fn test_e2e_persistence_answer_fields() {
    ...
    assert_eq!(answer_map.get("step-1").map(|a| a.value.as_str()), Some("User answer"));
    assert_eq!(answer_map.get("step-2").map(|a| a.value.as_str()), Some("AI suggestion"));
    assert_eq!(
...
fn test_e2e_persistence_multiple_restarts() {
    ...
    // First session
    {
        ...
        store.save_metadata(&metadata).expect("Failed to save metadata");
    }
...
fn test_e2e_persistence_concurrent_writes() {
    ...
    for i in 0..20 {
        let answer = create_test_answer(&format!("concurrent-{:03}", i), &format!("Answer {}", i));
        store.save_answer(&answer).expect("Failed to save answer");
    }
    ...
    let lattice = LatticeCache::with_current_timestamp("develop".to_string(), r#"{"test": "data"}"#.to_string());
    store.save_lattice_cache("develop", &lattice).expect("Failed to save lattice cache");
    ...
    let restored_lattice = store.get_lattice_cache("develop").expect("Failed to get lattice cache");
    assert!(restored_lattice.is_some());
...
fn test_e2e_persistence_empty_database() {
    ...
    let answers = store.load_answers().expect("Failed to load answers after restart");
    assert!(answers.is_empty());
}
...
fn test_e2e_persistence_answer_update() {
    ...
    store.save_answer(&answer1).expect("Failed to save initial answer");

    let answer2 = create_test_answer("update-test", "Updated value");
    store.save_answer(&answer2).expect("Failed to save updated answer");
...
fn test_e2e_persistence_answer_deletion() {
    ...
    let deleted = store.delete_answer("delete-2").expect("Failed to delete answer");
    assert!(deleted);

...
use clarity_web::app::pages::HomePage;
use clarity_web::components::quality::{QualityScoreBar, QualityScoreBarProps, MINIMUM_GATE};
use clarity_web::hooks::use_quality_score;
...
fn test_quality_score_calculation_with_answers() {
    ...
    if let Ok(score) = result {
        ...
        let completeness = score.get_dimension(clarity_web::lattice::quality::QualityDimension::Completeness);
...
fn test_quality_score_serialization_for_cache() {
    ...
    if let Ok(score) = result {
        ...
        if let Ok(deser_score) = deserialized {
...
fn test_ears_requirements_integration() {
    ...
    if let Ok(ears_output) = result {
        // Convert to quality module format
        let ears_refs: Vec<EarsRequirementRef> = ears_output
            .requirements
            .iter()
            .enumerate()
...
fn test_inversion_integration() {
    use clarity_web::lattice::inversion::{invert, InversionOutput};
    ...
    if let Ok(inversion_output) = result {
...
fn test_quality_score_passed_to_develop_phase() {
    ...
    if let Ok(score) = result {
        ...
        if let Ok(restored_score) = restored {
...
fn test_e2e_quality_gate_unlocks_develop() {
    ...
    let improved_score = calculate_quality(&quality_answers_improved, &improved_ears, &inversion).unwrap();
...
fn test_e2e_tooltip_explains_quality_gate() {
    ...
    let tooltip_message = if discover_complete && !passes_gate {
        Some(format!(
            "Quality score must be at least {} to proceed",
            MINIMUM_GATE
        ))
    } else {
        None
    ...
    assert!(
        ...
        "Tooltip should be shown when Discover complete but quality low"
    );
...
fn test_e2e_complete_user_journey() {
    ...
    if mid_score.overall < MINIMUM_GATE {
        ...
        assert!(
            !(!is_phase_done("discover", &answers) && !final_score.passes(MINIMUM_GATE)),
            "Develop should be enabled"
        );
    }
}

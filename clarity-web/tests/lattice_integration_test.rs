...
use clarity_web::lattice::effects::{trace_effects, EffectsOutput};
...
fn test_ears_parsing_from_answers() {
    let answers = vec![
        ...
        create_answer("req2", "When the user is logged in, the system shall display the dashboard"),
        create_answer("req3", "During system startup, the system shall initialize all services"),
        create_answer("req4", "If the password is invalid, the system shall not grant access"),
        create_answer("req5", "Where the user has premium access, the system shall enable advanced features"),
    ];
...
Invalid permissions prevent feature access."#;
    ...
    assert!(output.dependency_graph.len() > 0);
...
fn test_partial_ars_requirements() {
    let answers = vec![
        ...
        create_answer("req3", "When the user is logged in, the system shall display the dashboard"),
    ];
...
fn test_effects_cycle_detection() {
    ...
    assert!(output.warnings.iter().any(|w| w.contains("circular dependency")));
}
...
Data validation enables system integrity."#;
    ...
    let auth_node = output.nodes.iter().find(|n| n.id.contains("Authentication"));
...
fn test_ars_all_pattern_types() {
    let answers = vec![
        ...
        create_answer("req2", "When the form is submitted, the system shall validate data"),
        create_answer("req3", "During processing, the system shall log all activities"),
        create_answer("req4", "If validation fails, the system shall not proceed"),
        create_answer("req5", "Where debug mode is enabled, the system shall show detailed logs"),
    ];

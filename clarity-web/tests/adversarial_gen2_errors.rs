...
fn test_quality_error_invalid_score_context() {
    ...
    match result {
        Err(QualityError::InvalidScore(msg)) => {
            ...
            assert!(msg.contains("255"), "Error should mention the invalid score");
        }
        _ => {
...
fn test_quality_error_chain_integrity() {
    ...
    match result {
        Ok(score) => {
            ...
            assert!(consistency.unwrap().score < 100, "Score should be reduced due to contradiction");

            let consistency_issues = score.get_issues(QualityDimension::Consistency);
            assert!(!consistency_issues.is_empty(), "Should have consistency issues");
...
fn test_storage_error_deserialization_recovery() {
    ...
    assert!(valid_result.unwrap().is_some(), "Valid answer should be retrievable");
...
"#;
    ...
    match result {
        Ok(config) => {
            assert_eq!(config.provider.provider, clarity_web::config::ai::ProviderType::Opencode);
...
fn test_quality_error_partial_failure_handling() {
    ...
    match result {
        Ok(score) => {
            ...
            assert!(!security_issues.is_empty(), "Should report missing security");

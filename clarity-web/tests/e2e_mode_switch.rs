...
async fn test_mode_preference_persists() {
    ...
    let metadata = ProjectMetadata::with_current_timestamp("guided".to_string(), "discover".to_string());
...
async fn test_mode_preference_update() {
    ...
    let metadata1 = ProjectMetadata::with_current_timestamp("guided".to_string(), "discover".to_string());
    storage.save_metadata(&metadata1).unwrap();
    
    // Update to express mode
    let metadata2 = ProjectMetadata::with_current_timestamp("express".to_string(), "discover".to_string());
...
async fn test_full_mode_switch_round_trip() {
    ...
    let guided_metadata = ProjectMetadata::with_current_timestamp("guided".to_string(), "discover".to_string());
    ...
    storage.save_answer(&clarity_web::types::Answer {
    ...
    let express_metadata = ProjectMetadata::with_current_timestamp("express".to_string(), "discover".to_string());
...
async fn test_mode_preference_survives_reload() {
    ...
    let metadata2 = ProjectMetadata::with_current_timestamp("express".to_string(), "define".to_string());
    storage.save_metadata(&metadata2).unwrap();
    
    // Verify update persisted
    let final = storage.get_metadata().unwrap().unwrap();
    assert_eq!(final.mode_preference, "express");
    assert_eq!(final.current_phase, "define");
}

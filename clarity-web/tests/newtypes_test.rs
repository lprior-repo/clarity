...
// Test scaffolding: `expect()` is the documented test-failure mechanism (matches
// the established pattern across `clarity-web/tests/*.rs`). Production code
// remains zero-unwrap; tests are the one place where panics are intentional.

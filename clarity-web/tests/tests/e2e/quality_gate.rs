...
fn test_low_quality_locks_develop() {
  ...
  assert!(
    ...
    "Score should indicate red/yellow color"
  );
...
fn test_quality_score_color_coding() {
  ...
  let medium_quality_answers: Vec<Answer> = medium_answers.iter().map(to_quality_answer).collect();
...
fn test_quality_bar_color_progression() {
  ...
  assert!(red_score.overall < 50, "Red zone score should be < 50");
...
fn test_dimension_breakdown_contribution() {
  ...
  for dimension in QualityDimension::all() {
    ...
    assert!(dim_score.is_some(), "Should have score for {:?}", dimension);

    if let Some(score) = dim_score {
      assert!(score.score <= 100, "{:?} score should be <= 100", dimension);
    }
  }

  // Overall should be average of dimensions
  let sum: u32 = quality_score
    .dimensions
    .iter()
...
fn test_develop_transition_after_quality_gate() {
  ...
  assert!(quality_score.overall >= 70, "Score should show green color");
}
...
fn test_issues_explain_quality_gaps() {
  ...
  assert!(!security_issues.is_empty(), "Should have security issues");
...
fn test_minimum_gate_constant() {
  ...
  assert!(score.passes(MINIMUM_GATE), "Score at threshold should pass");
  assert!(score.passes(69), "Score at 70 should pass 69 threshold");
  assert!(
    !score.passes(71),
    "Score at 70 should not pass 71 threshold"
  );

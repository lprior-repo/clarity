...
fn main() {
    ...
    let invalid_cases = [("", "empty"), ("bad/name", "separators"), (".hidden", "dot")];
    ...
    assert!(!project_dir.exists(), "Directory should not exist initially");

    let result = ensure_project_dir_exists(project_id);
    assert!(result.is_ok(), "Should create directory");
    assert!(project_dir.exists(), "Directory should exist after creation");
    ...
    assert!(db_path.starts_with(&project_dir), "DB should be under project dir");

...
use clarity_web::storage::{
    AnswerRecord, Confidence, ExtractionCache, LatticeCache, ProjectMetadata, tables,
    RedbStore,
};
...
fn create_test_answer_record(step_id: &str, value: &str, confidence: Confidence, ai_generated: bool) -> AnswerRecord {
    AnswerRecord::new(
        step_id.to_string(),
        value.to_string(),
...
fn count_table_records(db_path: &Path, table_name: &str) -> Result<usize, String> {
    ...
    let txn = db.begin_read().map_err(|e| format!("Failed to begin read: {}", e))?;
    let table_def: redb::TableDefinition<&str, &str> = redb::TableDefinition::new(table_name);

    match txn.open_table(table_def) {
        Ok(table) => {
            let mut count = 0;
            let mut iter = table.iter().map_err(|e| format!("Failed to iterate table: {}", e))?;
...
fn test_e2e_persistence_discover_phase_express_mode() {
    ...
    for answer in &answers {
        ...
        store.save_answer(&answer_for_save)
    ...
    let lattice_cache = LatticeCache::with_current_timestamp(
        "discover".to_string(),
        lattice_output.to_string(),
    );
    store.save_lattice_cache("discover", &lattice_cache)
    ...
    verify_database_file_exists(&db_path)
        .expect("Database file verification failed");
    ...
    let answer_count = count_table_records(&db_path, tables::ANSWERS)
        .expect("Failed to count answers");
    assert_eq!(
        answer_count, 5,
        "Expected 5 answers in database, found {}",
        answer_count
    );
    ...
    assert_eq!(
        metadata_count, 1,
        "Expected 1 metadata record in database, found {}",
        metadata_count
    );
    ...
    assert_eq!(
        lattice_count, 1,
        "Expected 1 lattice cache record in database, found {}",
        lattice_count
    );
    ...
    let loaded_answers = store.load_answers()
        .expect("Failed to load answers");
    assert_eq!(
        loaded_answers.len(), 5,
        "Expected to load 5 answers, got {}",
        loaded_answers.len()
    );

    // Verify metadata mode preference
    let loaded_metadata = store.get_metadata()
    ...
    let loaded_lattice = store.get_lattice_cache("discover")
    ...
    let store_reopened = RedbStore::open(&db_path)
        .expect("Failed to reopen database after restart");
    ...
    let restored_answers = store_reopened.load_answers()
        .expect("Failed to load answers after restart");
    assert_eq!(
        restored_answers.len(), 5,
    ...
    let restored_metadata = store_reopened.get_metadata()
    ...
    let restored_lattice = store_reopened.get_lattice_cache("discover")
...
fn test_e2e_persistence_answer_fields() {
    ...
    let loaded = store.load_answers()
        .expect("Failed to load answers");
    assert_eq!(loaded.len(), 3);

    // Check individual answers
    let answer_map: HashMap<_, _> = loaded
        .into_iter()
        .map(|a| (a.step_id.clone(), a))
        .collect();
...
fn test_e2e_persistence_multiple_restarts() {
    ...
    // Third session: Verify all data persisted
    {
        ...
        let answers = store.load_answers()
            .expect("Failed to load answers");
        assert_eq!(answers.len(), 2, "Should have 2 answers from multiple sessions");
...
fn test_e2e_persistence_concurrent_writes() {
    ...
    let lattice = LatticeCache::with_current_timestamp(
        "develop".to_string(),
        r#"{"test": "data"}"#.to_string(),
    );
    store.save_lattice_cache("develop", &lattice)
        .expect("Failed to save lattice cache");
    ...
    let restored_metadata = store.get_metadata()
        .expect("Failed to get metadata")
        .expect("No metadata found");
    assert_eq!(restored_metadata.mode_preference, "express");

    let restored_lattice = store.get_lattice_cache("develop")
        .expect("Failed to get lattice cache")
        .expect("No lattice cache found");
...
fn test_e2e_persistence_empty_database() {
    ...
    drop(store);
    let store = RedbStore::open(&db_path)
        .expect("Failed to reopen empty database");

    // Verify still empty
    let answers = store.load_answers()
        .expect("Failed to load answers after restart");
    assert!(answers.is_empty(), "Database should still be empty after restart");
}
...
fn test_e2e_persistence_extraction_cache() {
    ...
    store.save_extraction_cache("hash-abc123", &cache1)
        .expect("Failed to save extraction cache 1");
    store.save_extraction_cache("hash-xyz789", &cache2)
    ...
    let missing = store.get_extraction_cache("hash-missing")
...
fn test_e2e_persistence_answer_deletion() {
    ...
    store.save_answer(&create_test_answer("delete-1", "Keep me"))
        .expect("Failed to save answer 1");
    store.save_answer(&create_test_answer("delete-2", "Delete me"))
        .expect("Failed to save answer 2");
    store.save_answer(&create_test_answer("delete-3", "Keep me too"))
        .expect("Failed to save answer 3");

    // Delete one answer
    let deleted = store.delete_answer("delete-2")

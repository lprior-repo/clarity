...
fn test_extracted_fields_serialization_roundtrip() {
    use clarity_web::providers::{ExtractedFields, FieldExtraction, ExtractionMetadata, FieldType};
    use chrono::Utc;
    ...
    let deserialized: ExtractedFields = serde_json::from_str(&serialized).expect("Failed to deserialize");
...
fn test_quality_score_serialization_roundtrip() {
    use clarity_web::lattice::quality::{QualityScore, DimensionScore, QualityDimension};
    ...
    let deserialized: QualityScore = serde_json::from_str(&serialized).expect("Failed to deserialize");
...
fn test_extraction_context_serialization_roundtrip() {
    ...
    let deserialized: ExtractionContext = serde_json::from_str(&serialized).expect("Failed to deserialize");
...
fn test_ears_requirement_ref_serialization_roundtrip() {
    ...
    let deserialized: EarsRequirementRef = serde_json::from_str(&serialized).expect("Failed to deserialize");

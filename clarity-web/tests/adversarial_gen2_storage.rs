...
fn test_storage_concurrent_read_write() {
    ...
    for i in 0..10 {
        ...
        store.save_answer(&answer).expect("Failed to save initial answer");
    }
    ...
    assert!(all_answers.len() >= 10, "Should have at least initial answers");
...
fn test_storage_special_characters_in_values() {
    ...
    let special_cases = vec![
        ("quotes", create_answer("quotes", "Text with \"quotes\" and 'apostrophes'")),
        ("newlines", create_answer("newlines", "Line 1\nLine 2\rLine 3")),
        ("unicode", create_answer("unicode", "Unicode: 🎉 中文 العربية")),
        ("null", create_answer("null", "Text with \0 null byte")),
        ("backslash", create_answer("backslash", "Path: C:\\Users\\Test")),
    ];
...
fn test_storage_rapid_fire_operations() {
    ...
    let all_answers = store
        .get_all_answers()
        .expect("Failed to get all answers");

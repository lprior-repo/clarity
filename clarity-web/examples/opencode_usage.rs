...
use clarity_web::providers::{
  ExtractionContext, ExtractionError, ExtractionProvider, FieldType, OpenCodeProvider, SchemaField,
};
use serde_json::json;
...
async fn simple_extraction(provider: &OpenCodeProvider) -> Result<(), ExtractionError> {
  let text = r#"
        John Doe
  ...
  match provider.extract_fields(text, &context).await {
    Ok(result) => {
      ...
      println!(
        "Processing time: {}ms",
        result.metadata.processing_duration_ms
      );
      println!();
...
async fn schema_extraction(provider: &OpenCodeProvider) -> Result<(), ExtractionError> {
  let text = r#"
        INVOICE #12345
        Date: 2025-02-25
        Customer: Acme Corp
...
async fn health_check_example(provider: &OpenCodeProvider) -> Result<(), ExtractionError> {

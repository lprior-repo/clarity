...
use crate::providers::{
    ExtractionContext, ExtractionError, ExtractionProvider, ExtractedFields, FieldType,
...
impl From<ExtractionError> for TerminalError {
    fn from(error: ExtractionError) -> Self {
        match error {
            ...
            ExtractionError::ApiError { message, status_code } => Self::ServerError {
                message,
                status_code,
            },
...
impl ConnectionState {
    ...
    pub const fn on_failure(&self, retryable: bool) -> Self {
        match (self, retryable) {
            (Self::Connecting, true) | (Self::Reconnecting, true) => Self::Reconnecting,
            (Self::Connecting, false) | (Self::Reconnecting, false) => Self::Failed,
...
impl ConnectionStatus {
    ...
    pub fn success_rate(&self) -> f64 {
        let total = self.successful_requests.saturating_add(self.failed_requests);
        if total == 0 {
            return 1.0;
        }
...
impl OpenCodeTerminalClient {
    ...
    pub fn new(config: TerminalConfig) -> Result<Self, TerminalError> {
        ...
        Ok(Self {
            ...
            status: Arc::new(RwLock::new(ConnectionStatus::new(ConnectionState::Disconnected))),
            config,
        })
    }
            ...
            match self.provider.extract_fields(text, context).await {
                ...
                Err(e) => {
                    ...
                    let retryable = matches!(
                        terminal_error,
                        TerminalError::ConnectionFailed { retryable: true, .. }
                    ...
                    *self.status.write().await = ConnectionStatus::new(new_state)
                        .with_failure(error_msg);
    ...
    async fn extract_with_schema(
        ...
        while attempts < self.config.max_retries {
            ...
            match self.provider.extract_fields_with_schema(text, schema, context).await {
                Ok(fields) => {
                    let latency = start.elapsed().as_millis().try_into().unwrap_or(u64::MAX);
    ...
    async fn validate_straw_man(
        ...
        let schema = vec![
            SchemaField {
                ...
                required: true,
                description: Some(
                    "True if the persona acts against their own motivations or self-interest"
                        .to_string(),
                ),
                options: None,
            },
            SchemaField {
                ...
                required: true,
                description: Some(
                    "True if the persona magically loves everything without discernment"
                        .to_string(),
                ),
                options: None,
            },
            ...
            SchemaField {
                ...
                required: true,
                description: Some(
                    "True if the persona has developer-level system knowledge or mental models"
                        .to_string(),
                ),
                options: None,
            },
            SchemaField {
                ...
                required: false,
                description: Some(
                    "Specific suggestions for fixing detected traps. Be concrete and actionable."
                        .to_string(),
                ),
                options: None,
            },
        ...
        self.extract_with_schema(persona_text, &schema, &context).await
    }
        ...
        match result {
            Ok(()) => {
                ...
                *self.status.write().await =
                    status_guard.with_state(ConnectionState::Connected);
...
impl TranscriptProcessor {
    ...
    pub async fn process_transcript(
        ...
        // Extract problem
        let problem_result = self
            .client
            .extract_problem(&original_prompt)
            .await?;
        let problem = extract_field_value(&problem_result, "problem")
            .unwrap_or_else(|| original_prompt.clone());

        // Extract persona
        let persona_result = self
            .client
            .extract_persona(&original_prompt)
            .await?;
        let persona = extract_field_value(&persona_result, "persona")
            .unwrap_or_default();

        // Extract solution
        let solution_result = self
            .client
            .extract_solution(&original_prompt)
            .await?;
        let solution = extract_field_value(&solution_result, "solution")
            .unwrap_or_default();

        // Extract non-persona
        let nonpersona_result = self
            .client
            .extract_nonpersona(&original_prompt)
            .await?;
        let nonpersona = extract_field_value(&nonpersona_result, "nonpersona")
            .unwrap_or_default();
...
mod tests {
    ...
    fn test_connection_state_on_success() {
        assert_eq!(ConnectionState::Connecting.on_success(), ConnectionState::Connected);
        assert_eq!(ConnectionState::Reconnecting.on_success(), ConnectionState::Connected);
        assert_eq!(ConnectionState::Connected.on_success(), ConnectionState::Connected);
    }
    ...
    fn test_connection_status_with_failure() {
        let status = ConnectionStatus::disconnected()
            .with_failure("Connection refused".to_string());
    ...
    fn test_terminal_error_from_extraction_error() {
        ...
        let extraction_error = ExtractionError::RateLimited { retry_after_seconds: 60 };
        let terminal_error = TerminalError::from(extraction_error);
        ...
        let terminal_error = TerminalError::from(extraction_error);
        assert!(matches!(terminal_error, TerminalError::AuthenticationFailed(_)));
    }
    ...
    fn test_processed_transcript_serialization() {
        ...
        let deserialized: Result<ProcessedTranscript, _> =
...
mod mock_client {
    ...
    async fn test_mock_client_failure() {
        ...
        assert!(matches!(
            error,
            TerminalError::ConnectionFailed { retryable: false, .. }
        ));
    }
    ...
    async fn test_transcript_processor() {
        ...
        let transcript = InterrogationTranscript::from_prompt(
            "Build a todo app for busy professionals".to_string(),
        );
        ...
        assert_eq!(processed.original_prompt, "Build a todo app for busy professionals");

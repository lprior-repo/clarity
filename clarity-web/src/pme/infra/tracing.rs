    ...
    pub fn parse(s: &str) -> Result<Self, TracingError> {
        ...
        for (i, chunk) in s.as_bytes().chunks(2).enumerate() {
            ...
            let hex = std::str::from_utf8(chunk)
                .map_err(|_| TracingError::InvalidTraceId(s.to_string()))?;
...
impl SpanId {
    ...
    pub fn parse(s: &str) -> Result<Self, TracingError> {
        ...
        for (i, chunk) in s.as_bytes().chunks(2).enumerate() {
            ...
            let hex = std::str::from_utf8(chunk)
                .map_err(|_| TracingError::InvalidSpanId(s.to_string()))?;
            bytes[i] = u8::from_str_radix(hex, 16)
                .map_err(|_| TracingError::InvalidSpanId(s.to_string()))?;
        }
...
impl SpanBuilder {
    ...
    pub fn with_int_attribute(mut self, key: impl Into<String>, value: i64) -> Self {
        self.attributes.insert(key.into(), AttributeValue::Int(value));
        self
    }
...
mod tests {
    ...
    fn test_trace_context_traceparent_invalid() {
        ...
        assert!(TraceContext::from_traceparent("01-00112233445566778899aabbccddeeff-0011223344556677-01").is_err()); // Invalid version
    }
    ...
    fn test_span_with_parent() {
        ...
        let span = Span::new("child", TraceId::generate(), SpanId::generate(), "svc")
    ...
    fn test_trace_summary() {
        ...
        let _child = tracer.start_child("child", &Span::new("dummy", TraceId::generate(), SpanId::generate(), "test"));

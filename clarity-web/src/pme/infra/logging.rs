...
impl LogEntry {
  ...
  pub fn to_json(&self) -> Result<String, LoggingError> {
    serde_json::to_string(self).map_err(|e| LoggingError::SerializationFailed(e.to_string()))
  }

  /// Convert to pretty JSON string (for development)
  pub fn to_json_pretty(&self) -> Result<String, LoggingError> {
    serde_json::to_string_pretty(self).map_err(|e| LoggingError::SerializationFailed(e.to_string()))
  }
}
...
impl StructuredLogger {
  ...
  pub fn create_entry(
    ...
    LogEntry::new(level, message, &self.config.service, &self.config.version).with_context(context)
  }
  ...
  pub fn trace(&self, context: LogContext, message: impl Into<String>, data: &JsonValue) {
    if self.should_log(LogLevel::Trace, context.component.as_deref()) {
      let entry = self
        .create_entry(LogLevel::Trace, message, context)
  ...
  pub fn debug(&self, context: LogContext, message: impl Into<String>, data: &JsonValue) {
    if self.should_log(LogLevel::Debug, context.component.as_deref()) {
      let entry = self
        .create_entry(LogLevel::Debug, message, context)
        .with_data(data.clone());
      self.output(entry);
    }
  }
  ...
  pub fn info(&self, context: LogContext, message: impl Into<String>, data: &JsonValue) {
    if self.should_log(LogLevel::Info, context.component.as_deref()) {
      let entry = self
        .create_entry(LogLevel::Info, message, context)
        .with_data(data.clone());
      self.output(entry);
    }
  }
  ...
  pub fn warn(&self, context: LogContext, message: impl Into<String>, data: &JsonValue) {
    if self.should_log(LogLevel::Warn, context.component.as_deref()) {
      let entry = self
        .create_entry(LogLevel::Warn, message, context)
        .with_data(data.clone());
      self.output(entry);
    }
  }
...
mod tests {
  ...
  fn test_log_context_validate() {
    let ctx = LogContext::new().with_request_id("req-1");
  ...
  fn test_logger_should_log() {
    let logger = StructuredLogger::new().with_min_level(LogLevel::Info);
  ...
  fn test_error_info() {
    ...
    let entry =
      LogEntry::new(LogLevel::Error, "Error occurred", "svc", "1.0").with_error(error_info);

    ...
    pub fn report(&self) -> CoverageReport {
        let module_reports: Vec<ModuleReport> = self
            ...
            .map(|(path, module)| ModuleReport {
                ...
                uncovered_items: module.uncovered_items().into_iter().map(String::from).collect(),
            })
            .collect();
...
#[must_use]
pub fn assert_in_range<T: std::fmt::Debug + PartialOrd>(value: &T, min: &T, max: &T) -> AssertionResult {

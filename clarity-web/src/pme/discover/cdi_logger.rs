...
impl SignalStrength {
  ...
  pub const fn description(&self) -> &'static str {
    match self {
      Self::High => {
        "User volunteered this information without prompting - strong validation signal"
      }
...
impl CdiLogger {
    ...
    let by_strength: Vec<(SignalStrength, usize)> = [
      ...
      (
        ...
        entries
          .iter()
          .flat_map(|e| &e.signals)
          .filter(|s| s.strength == SignalStrength::Mixed)
          .count(),
      ),
      (
        SignalStrength::Low,
        entries
          .iter()
          .flat_map(|e| &e.signals)
          .filter(|s| s.strength == SignalStrength::Low)
          .count(),
      ),
    ]
    .into_iter()
  ...
  fn generate_recommendations(funnel: &CdiFunnel, analysis: &SignalAnalysis) -> Vec<String> {
    ...
    if funnel.contact_success_rate() < 0.3 {
      recommendations
        .push("Low contact success rate - review outreach messaging and channels".to_string());
    }

    if funnel.interview_completion_rate() < 0.5 {
      recommendations.push(
        "Low interview completion rate - consider shorter interview format or better scheduling"
          .to_string(),
      );
    }
    ...
    if funnel.high_signal_rate() < 0.3 {
      recommendations.push(
        "Low high-signal rate - focus on open-ended questions to elicit volunteered information"
          .to_string(),
      );
    }
    ...
    if problem_count > 0 && outcome_count == 0 {
      recommendations.push(
        "Problems identified but no desired outcomes - ask users what success looks like"
          .to_string(),
      );
    }
    ...
    if funnel.completed_interviews < 5 {
      ...
      recommendations.push(
        "10+ interviews but few high-signal results - may need to refine target segment"
          .to_string(),
      );
    }
  ...
  pub fn segment_performance(entries: &[CdiEntry]) -> Vec<(String, f64)> {
    entries
      ...
      .filter_map(|e| {
        e.segment
          .as_ref()
          .map(|s| (s.clone(), e.total_signal_score()))
      })

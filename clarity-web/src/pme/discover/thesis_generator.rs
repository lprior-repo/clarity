...
impl Thesis {
    ...
    pub fn key_terms(&self) -> Vec<String> {
        let stop_words = ["the", "a", "an", "is", "are", "will", "be", "to", "for", "and", "or"];
...
impl FailureCategory {
    ...
    pub const fn description(&self) -> &'static str {
        match self {
            Self::ProblemDoesntExist => "The problem you're solving may not actually exist or be important enough",
    ...
    fn generate_antithesis_for_category(
        ...
        let (statement, reasoning) = match category {
            ...
            FailureCategory::ResourceConstraints => {
                ...
                let reasoning = "Funding, talent, time, or other resources may be inadequate to reach product-market fit".to_string();
                (statement, reasoning)
            }
        };
    ...
    fn generate_recommendations(thesis: &Thesis, antitheses: &[Antithesis]) -> Vec<String> {
        ...
        if thesis.assumptions.iter().any(|a| !a.is_empty()) {
            recommendations.push(
                "Document and validate each assumption underlying the thesis".to_string(),
            );
        }
        ...
        if thesis.confidence > 0.8 && !antitheses.is_empty() {
            recommendations.push(
                "High confidence warrants extra scrutiny - ensure it's based on evidence not optimism".to_string(),
            );
        }
...
mod tests {
    ...
    fn test_antithesis_creation() {
        ...
        assert_eq!(antithesis.failure_category, FailureCategory::BusinessModelFailure);
    ...
    fn test_validation_status_determination() {
        ...
        let high_risk = vec![
            Antithesis::new("Test".to_string(), "Reason".to_string(), FailureCategory::NoAdoption)
                .with_probability(0.8),
            Antithesis::new("Test".to_string(), "Reason".to_string(), FailureCategory::BusinessModelFailure)
    ...
    fn test_confidence_clamping() {
        let thesis = Thesis::new("Test".to_string(), "Rationale".to_string())
            .with_confidence(1.5); // Invalid, should be clamped

...
impl HumanLimitation {
    ...
    pub const fn all() -> [Self; 5] {
        [Self::Lazy, Self::Distracted, Self::RiskAverse, Self::Impatient, Self::Forgetful]
    }
...
impl Persona {
    ...
    pub fn new(name: String) -> Self {
        Self {
            ...
            limitations: HumanLimitation::all()
                .iter()
                .map(|&l| (l, 0.5))
                .collect(),
            goals: Vec::new(),
...
impl PersonaForge {
    ...
    pub fn validate(persona: &Persona) -> ValidationResult {
        ...
        if avg_limitation < 0.3 {
            issues.push(ValidationIssue {
                ...
                description: "Human limitations are too low - this may be a 'Straw Man' persona".to_string(),
                severity: 0.5,
            });
            suggestions.push("Increase limitation severities to reflect realistic human behavior".to_string());
        }
        ...
        if persona.resources.budget < 0.2 && persona.decision_authority > 0.8 {
            issues.push(ValidationIssue {
                ...
                description: "Low budget but high decision authority may indicate an unrealistic persona".to_string(),
                severity: 0.4,
            });
        }
        ...
        if realism_score < 0.7 {
            suggestions.push("Consider adding more context about daily workflow and environment".to_string());
        }
    ...
    fn check_irrational_actor(persona: &Persona) -> Vec<String> {
        ...
        if persona.get_limitation(HumanLimitation::Impatient) < 0.3
            && persona.resources.time < 0.3
        {
            warnings.push(
                "Unrealistically patient persona with very limited time".to_string(),
            );
        }
        ...
        {
            warnings.push(
                "Risk-seeking decision maker with limited budget is unusual".to_string(),
            );
        }

        // Check: No forgetfulness but low tech comfort
        if persona.get_limitation(HumanLimitation::Forgetful) < 0.2
            && persona.tech_comfort < 0.3
        {
            warnings.push(
                "Perfect memory with low tech comfort may indicate unrealistic persona".to_string(),
            );
        }

        // Check: Super-human across all limitations
        let low_limitation_count = persona
            .limitations
            .iter()
            .filter(|(_, s)| *s < 0.2)
            .count();
    ...
    fn calculate_realism_score(issues: &[ValidationIssue], persona: &Persona) -> f64 {
        ...
        let completeness_bonus = {
            let mut bonus = 0.0;
            if !persona.demographics.is_empty() { bonus += 0.05; }
            if persona.goals.len() >= 2 { bonus += 0.05; }
            if persona.pain_points.len() >= 2 { bonus += 0.05; }
            if !persona.current_behaviors.is_empty() { bonus += 0.05; }
            bonus
        };
    ...
    pub fn generate_design_recommendations(persona: &Persona) -> Vec<String> {
        ...
        if persona.resources.time < 0.3 {
            recommendations.push(
                "Persona has limited time - prioritize quick wins and efficiency".to_string(),
            );
        }
...
mod tests {
    ...
    fn test_persona_forge_validate_straw_man() {
        ...
        assert!(result.issues.iter().any(|i| i.category == IssueCategory::InsufficientLimitations));
    }
    ...
    fn test_persona_forge_contradiction_detection() {
        ...
        assert!(result.issues.iter().any(|i| i.category == IssueCategory::Contradictory));
    }
    ...
    fn test_limitation_severity_clamping() {
        let persona = Persona::new("Test".to_string())
            .with_limitation(HumanLimitation::Lazy, 1.5); // Invalid

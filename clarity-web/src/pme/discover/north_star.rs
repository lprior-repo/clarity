...
impl EdgeCase {
  ...
  pub fn needs_attention(&self) -> bool {
    matches!(
      self.handling_status,
      HandlingStatus::Unhandled | HandlingStatus::PartiallyHandled
    ) && self.severity > 0.5
  }
}
...
impl Scenario {
  ...
  pub fn has_discovery(&self) -> bool {
    self
      .timeline
      .iter()
      .any(|e| e.event_type == EventType::Discovery)
  ...
  pub fn ordered_events(&self) -> Vec<&TimelineEvent> {
    self.timeline.iter().sorted_by_key(|e| e.order).collect()
  }
}
...
impl NorthStarBuilder {
  ...
  fn simulate(scenario: &Scenario) -> SimulationResult {
    ...
    if !scenario.has_discovery() {
      ...
      recommendations
        .push("Define how users discover the product - this is critical for adoption".to_string());
    }
    ...
    if coherence_score < 0.7 {
      recommendations
        .push("Scenario has coherence issues - address plot holes before proceeding".to_string());
    }
...
mod tests {
  ...
  fn test_edge_case_creation() {
    let edge_case = EdgeCase::new(
      "Network Error".to_string(),
      "User loses connection".to_string(),
    )

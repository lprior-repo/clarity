...
impl VorpScore {
  ...
  pub fn recommendations(&self) -> Vec<String> {
    ...
    if self.value_score < 0.5 {
      recommendations.push("Focus on demonstrating clear, measurable value to users".to_string());
    }
    if self.obvious_score < 0.5 {
      recommendations
        .push("Make the value proposition immediately apparent to new users".to_string());
    }
    if self.real_score < 0.5 {
      recommendations.push("Validate that users and problem are real through research".to_string());
    }
    if self.possible_score < 0.5 {
      recommendations.push("Assess technical and resource feasibility more carefully".to_string());
    }
...
impl PrioritizedItem {
  ...
  pub fn new(id: String, title: String) -> Self {
    Self {
      ...
      brutal_truth_scores: BrutalTruth::all().iter().map(|&t| (t, 0.5)).collect(),
      priority: 50,
      risk: 50,
      category: String::new(),
  ...
  pub fn calculate_adjusted_priority(&self) -> u8 {
    ...
    let truth_avg = self.brutal_truth_scores.iter().map(|(_, s)| s).sum::<f64>()
...
impl BrutalTruthsPrioritizer {
  ...
  pub fn quick_prioritize(
    ...
    let prioritized: Vec<PrioritizedItem> = items
      ...
      .map(|(_idx, (id, title))| {
        ...
        let sustain_score = if contains_any(
          &lower,
          &["long-term", "sustainable", "ongoing", "continuous"],
        ) {
          0.7
        } else {
          0.5
  ...
  fn calculate_stats(items: &[PrioritizedItem]) -> BrutalTruthsStats {
    let total = items.len();
    let avg_priority = items.iter().map(|i| f64::from(i.priority)).sum::<f64>()
      / f64::from(u8::try_from(total).unwrap_or(1));
    let avg_vorp = items.iter().map(|i| i.vorp_score.score).sum::<f64>()
      / f64::from(u8::try_from(total).unwrap_or(1));
    let avg_risk = items.iter().map(|i| f64::from(i.risk)).sum::<f64>()
      / f64::from(u8::try_from(total).unwrap_or(1));
...
mod tests {
  ...
  fn test_item_recommendations_generated() {
    ...
    assert!(
      recs
        .iter()
        .any(|r| r.to_lowercase().contains("value") || r.to_lowercase().contains("risk")),
  ...
  fn test_vorp_with_replacement() {
    let score =
      VorpScore::new(0.7, 0.7, 0.7, 0.7).with_replacement("Competitor Product X".to_string());

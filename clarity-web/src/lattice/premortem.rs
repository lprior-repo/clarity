...
fn analyze_technical_failures(solution: &str, _constraints: &[String]) -> Vec<FailureScenario> {
    ...
    let integration_likelihood = if contains_any(solution, &["api", "integration", "external", "service"]) {
        Likelihood::Possible
    } else {
        Likelihood::Unlikely
    };
    ...
    scenarios.push(FailureScenario::new(
        ...
        "External dependencies change their API, become unavailable, or have unexpected downtime".to_string(),
...
fn analyze_user_failures(solution: &str, _constraints: &[String]) -> Vec<FailureScenario> {
    ...
    // Poor adoption
    let adoption_likelihood = if contains_any(solution, &["new", "tool", "platform", "system", "workflow"]) {
        Likelihood::Possible
    } else {
        Likelihood::Unlikely
    };
...
fn analyze_business_failures(solution: &str, _constraints: &[String]) -> Vec<FailureScenario> {
    ...
    // Cost overruns
    let cost_likelihood = if contains_any(solution, &["scale", "infrastructure", "cloud", "storage", "compute"]) {
        Likelihood::Possible
    } else {
        Likelihood::Unlikely
    };
...
fn analyze_security_failures(solution: &str, _constraints: &[String]) -> Vec<FailureScenario> {
    ...
    // Data breach
    let breach_likelihood = if contains_any(solution, &["data", "user", "authentication", "api", "network"]) {
        Likelihood::Possible
    } else {
        Likelihood::Unlikely
    };
    ...
    let auth_likelihood = if contains_any(solution, &["authentication", "authorization", "user", "access", "login"]) {
        Likelihood::VeryLikely
    } else {
        Likelihood::Possible
    };
...
mod tests {
    ...
    fn test_generate_premortem_creates_scenarios() {
        ...
        let constraints = vec
!["Must handle 10k concurrent users", "Data must be encrypted"];
...
, "Should generate at least one scenario");
    ...
    fn test_generates_reasonable_scenarios() {
        ...
        let constraints = vec
!["Must be secure", "Low latency required"];
    ...
    fn test_covers_all_categories() {
        ...
        let constraints = vec
!["Must scale globally", "GDPR compliance required"];
...
!["Must process 1TB/day"];
        ...
        for scenario in &output.scenarios {
            ...
            for mitigation in &scenario.mitigation {
                assert!(
                    !mitigation.is_empty(),
                    "Mitigation should not be empty"
                );
                assert!(
                    mitigation.len() > 10,
...
!["Must be scalable and secure"];
        ...
        assert!(!by_category.is_empty(), "Should have scenarios grouped by category");

        for (category, scenarios) in by_category {
            assert!(!scenarios.is_empty(), "Category {category:?} should have scenarios");
...
!["Must handle load"];
        ...
        assert!(!tech_scenarios.is_empty(), "Should detect technical failures");
    }
...
!["Must be secure"];
        ...
        assert!(!security_scenarios.is_empty(), "Should detect security failures");
...
!["Must scale automatically"];
        ...
        assert!(!business_scenarios.is_empty(), "Should detect business failures");
    }
...
!["Must be secure"];
        ...
        for scenario in &output.scenarios {
            let unique_mitigations: std::collections::HashSet<_> =
                scenario.mitigation.iter().collect();
            assert_eq!(

...
fn parse_requirement_patterns(input: &str) -> Result<EarsRequirement, EarsError> {
    ...
    parse_ubiquitous(input)
        .ok_or_else(|| EarsError::UnrecognizedPattern(input.to_string()))
}
...
fn parse_unwanted(input: &str) -> Option<EarsRequirement> {
    ...
    lower
        .find(CONDITION_PREFIX)
        .and_then(|condition_start| {
            // Find the negation pattern in the lowercase string
            let negation_pattern_start = lower.find(NEGATION_PATTERN)?;
            let condition = input[condition_start + CONDITION_PREFIX.len()
                ..negation_pattern_start]
                .trim()
                .trim_end_matches(',')
                .trim()
                .to_string();
...
pub fn parse_requirements(input: &str) -> EarsOutput {
    input
        ...
        .fold(EarsOutput::new(), |mut output, line| {
...
mod tests {
    ...
    fn test_unrecognized_pattern() {
        ...
        assert!(matches!(
            result,
            Err(EarsError::UnrecognizedPattern(_))
        ));
    }
...
During system startup, the system shall initialize all services"#;
    ...
    fn test_actor_extraction() {
        ...
        assert_eq!(req, EarsRequirement::Ubiquitous {
            actor: "system".to_string(),
            action: "process data".to_string(),
        });
    }
    ...
    fn test_action_extraction_all_patterns() {
        ...
        // Optional
        let req5 =
            parse_requirement("Where configured, the system shall use HTTPS").unwrap();
        assert_eq!(
            req5,
            EarsRequirement::Optional {
                actor: "system".to_string(),

...
mod tests {
    ...
    fn test_extract_assumptions_minimum_three() {
        ...
        assert!(assumptions.len() >= 3, "Expected at least 3 assumptions, got {}", assumptions.len());
    }
    ...
    fn test_realistic_scenario() {
        ...
        assert!(output.total_challenges() >= 3, "Expected at least 3 challenges, got {}", output.total_challenges());

        // Should find critical issues with "always" and "guarantee"
        assert!(output.critical_count >= 1, "Expected at least 1 critical challenge");
        ...
        let patterns = output
            ...
            .unique()
            .collect::<Vec<_>>();
        assert!(patterns.len() >= 2, "Expected at least 2 different patterns");

        // Quality should reflect the critical issues found
        assert!(output.quality_score < 90, "Quality score should be reduced by critical challenges");
    }
    ...
    fn test_extract_assumptions_handles_whitespace() {
        ...
        assert!(assumptions.iter().all(|a| !a.starts_with(' ') && !a.ends_with(' ')));
    }

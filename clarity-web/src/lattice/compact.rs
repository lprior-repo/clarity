...
impl CompactSummary {
    ...
    pub fn limit_bullets(&self, max_per_section: usize) -> Self {
        Self {
            ...
            solution: self.solution.iter().take(max_per_section).cloned().collect(),
            requirements: self.requirements.iter().take(max_per_section).cloned().collect(),
            constraints: self.constraints.iter().take(max_per_section).cloned().collect(),
...
impl CompactOutput {
    ...
    pub fn to_agent_format(&self) -> String {
        ...
        output.push_str(&format!("---\n**Artifacts processed**: {}", self.artifact_count));
...
impl CompactAnswer {
    ...
    fn step_contains(&self, keyword: &str) -> bool {
        self.step_id.to_lowercase().contains(&keyword.to_lowercase())
    }
}
...
pub fn compact_artifacts(all_answers: Vec<CompactAnswer>) -> Result<CompactOutput, CompactError> {
    match all_answers.is_empty() {
        ...
        false => {
            ...
            Ok(CompactOutput::new(
                summary.limit_bullets(5),
                artifact_count,
            ))
        }
    }
...
fn build_summary(answers: Vec<CompactAnswer>) -> CompactSummary {
    ...
    let prioritized = answers
        .into_iter()
        .filter_map(|answer| {
            answer.phase().map(|phase| (phase, answer))
        })
...
mod tests {
    ...
    fn test_phase_from_step_id() {
        assert_eq!(Phase::from_step_id("discover-problem"), Some(Phase::Discover));
        assert_eq!(Phase::from_step_id("define-requirements"), Some(Phase::Define));
    ...
    fn test_phase_from_step_id_with_underscore() {
        assert_eq!(Phase::from_step_id("discover_problem"), Some(Phase::Discover));
        assert_eq!(Phase::from_step_id("define_requirements"), Some(Phase::Define));
    }
    ...
    fn test_serialization() {
        ...
        let deserialized: CompactSummary =
    ...
    fn test_compact_output_serialization() {
        ...
        let deserialized: CompactOutput =
            serde_json::from_str(&json).expect("Failed to deserialize");
    ...
    fn test_phase_serialization() {
        let phases = vec![Phase::Discover, Phase::Define, Phase::Develop, Phase::Deliver];

        for phase in phases {
            let json = serde_json::to_string(&phase).expect("Failed to serialize");
            let deserialized: Phase =
                serde_json::from_str(&json).expect("Failed to deserialize");

...
impl QualityIssue {
    ...
    pub const fn suggestion(&self) -> &'static str {
        match self {
            Self::MissingErrorTests => {
                "Add verification tests for error conditions and failure cases"
            }
            Self::MissingAuthenticationTest => {
...
impl QualityReport {
    ...
    pub fn new(
        ...
        let overall_score =
            calculate_overall_score_from_values(coverage_score, clarity_score, testability_score, ai_readiness_score);
...
fn check_invariants_tested(spec: &Spec) -> bool {
    ...
    let postcond_mentions_invariant = spec.features.iter().any(|f| {
        f.behaviors.iter().any(|b| {
            b.postconditions.iter().any(|p| p.to_lowercase().contains("invariant"))
        })
    });
...
fn calculate_verification_ratio(spec: &Spec) -> f64 {
    ...
    let with_verification = spec
        .features
        .iter()
        .map(|f| f.behaviors.iter().filter(|b| b.verification.is_some()).count())
        .sum::<usize>();
...
fn count_vague_language(spec: &Spec) -> u8 {
    let vague_words = [
        "maybe", "perhaps", "probably", "might", "could", "somehow", "something", "stuff",
        "things", "etc", "and so on", "roughly", "approximately", "usually", "typically",
...
fn calculate_precondition_ratio(spec: &Spec) -> f64 {
    ...
    let with_precond = spec
        .features
        .iter()
        .map(|f| f.behaviors.iter().filter(|b| !b.preconditions.is_empty()).count())
        .sum::<usize>();
...
mod tests {
    ...
    fn test_analyze_spec_with_comprehensive_content() {
        ...
        let verification = Verification::new("unit_test".to_string(), "Test with error cases".to_string())
    ...
    fn test_check_invariants_tested_with_verification() {
        let mut spec = create_minimal_spec();
        spec.add_invariant(Invariant::new("test".to_string(), "Test invariant".to_string()));

        let verification =
            Verification::new("unit_test".to_string(), "Verify invariant holds".to_string());
    ...
    fn test_calculate_verification_ratio() {
        ...
        let feature =
            create_feature_with_behaviors("test", vec![behavior1, behavior2]);
        spec.add_feature(feature).expect("should add feature");
    ...
    fn test_count_vague_language() {
        let spec = create_minimal_spec()
            .with_description("This maybe works sometimes".to_string());
    ...
    fn test_collect_testability_issues() {
        ...
        assert!(report.issues.contains(&QualityIssue::MissingPreconditions)

...
const STATE_CHANGE_KEYWORDS: &[&str] = &[
    "create", "update", "delete", "remove", "modify", "insert", "save",
    "write", "destroy", "archive", "restore", "purge", "reset", "clear",
    "set", "assign", "transfer", "move", "copy", "clone", "merge",
];

/// Keywords indicating notification operations
const NOTIFICATION_KEYWORDS: &[&str] = &[
    "email", "notify", "send", "alert", "webhook", "callback", "broadcast",
    "publish", "emit", "push", "sms", "message", "mail", "notification",
    "subscribe", "dispatch", "trigger", "announce", "signal",
];

/// Keywords indicating cascade operations
const CASCADE_KEYWORDS: &[&str] = &[
    "cascade", "propagate", "affect", "related", "dependent", "linked",
    "associated", "connected", "chain", "ripple", "downstream", "upstream",
    "sync", "synchronize", "replicate", "mirror",
];

/// Keywords indicating race condition risks
const RACE_CONDITION_KEYWORDS: &[&str] = &[
    "concurrent", "parallel", "simultaneous", "atomic", "lock", "mutex",
    "race", "thread", "async", "await", "concurrent", "conflict",
    "exclusive", "shared", "synchronize", "volatile", "interleave",
];

/// Keywords indicating rollback requirements
const ROLLBACK_KEYWORDS: &[&str] = &[
    "transaction", "rollback", "revert", "undo", "compensate", "recover",
    "restore", "backup", "snapshot", "version", "history", "audit",
    "reversible", "idempotent", "replay",
];

/// Keywords indicating high severity
const HIGH_SEVERITY_KEYWORDS: &[&str] = &[
    "critical", "severe", "urgent", "important", "essential", "vital",
    "irreversible", "permanent", "destructive", "dangerous", "sensitive",
    "secure", "encrypt", "auth", "permission", "admin", "root",
];

/// Keywords indicating critical severity
const CRITICAL_SEVERITY_KEYWORDS: &[&str] = &[
    "emergency", "fatal", "catastrophic", "unrecoverable", "corrupt",
    "breach", "expose", "leak", "compromise", "invalidate", "destroy",
    "wipe", "eliminate", "terminate", "abort",
];
...
impl EffectsSummary {
    ...
    pub fn from_effects(effects: &[Effect]) -> Self {
        let counts = effects
            .iter()
            .fold(HashMap::new(), |mut acc, effect| {
                *acc.entry(effect.effect_type).or_insert(0) += 1;
                acc
            });

        let max_severity = effects
            .iter()
            .map(|e| e.severity)
            .max();
    ...
    fn generate_summary_text(summary: &EffectsSummary, name: &str) -> String {
        ...
        let parts: Vec<String> = [
            (summary.state_changes > 0)
                .then(|| format!("{} state change(s)", summary.state_changes)),
            (summary.notifications > 0)
                .then(|| format!("{} notification(s)", summary.notifications)),
            (summary.cascades > 0)
                .then(|| format!("{} cascade(s)", summary.cascades)),
            (summary.race_conditions > 0)
                .then(|| format!("{} race condition(s)", summary.race_conditions)),
            (summary.rollbacks > 0)
                .then(|| format!("{} rollback(s)", summary.rollbacks)),
        ]
        .into_iter()
        .flatten()
        .collect();
...
impl SpecEffectsReport {
    ...
    fn generate_overall_summary(
        ...
        let behaviors_with_effects = reports
            .iter()
            .filter(|r| !r.effects.is_empty())
            .count();
...
mod tests {
    ...
    fn test_contains_keywords_match() {
        assert!(contains_keywords("Create a new user", STATE_CHANGE_KEYWORDS));
        assert!(contains_keywords("Send email notification", NOTIFICATION_KEYWORDS));
        assert!(contains_keywords("CREATE", STATE_CHANGE_KEYWORDS)); // case-insensitive
    }

    #[test]
    fn test_contains_keywords_no_match() {
        assert!(!contains_keywords("Read only operation", STATE_CHANGE_KEYWORDS));
    ...
    fn test_analyze_behavior_cascade() {
        ...
        let has_cascade = effects
            .iter()
            .any(|e| e.effect_type == EffectType::Cascade);
        assert!(has_cascade);
    }
    ...
    fn test_analyze_behavior_critical_severity() {
        ...
        let has_critical = effects.iter().any(|e| e.severity == EffectSeverity::Critical);
        assert!(has_critical);
    }

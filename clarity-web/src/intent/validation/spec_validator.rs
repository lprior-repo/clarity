        ...
        if let Some(neighbors) = self.adjacency.get(node) {
            for neighbor in neighbors {
                if !visited.contains(neighbor) {
                    if let Some(cycle) =
                        self.dfs_cycle_detect(neighbor, visited, recursion_stack, path)
                    {
                        return Some(cycle);
                    }
...
impl SpecValidator {
    ...
    fn validate_required_fields(&self, spec: &Spec, result: &mut ValidationResult) {
        if spec.name.trim().is_empty() {
            result.add_error(SpecValidationError::MissingRequiredField("spec.name".to_string()));
        }
    }
        ...
        for (index, feature) in spec.features.iter().enumerate() {
            if let Some(&first_index) = seen.get(feature.name.as_str()) {
                result.add_error(SpecValidationError::DuplicateBehavior {
                    behavior_a: format!("features[{}].{}", first_index, spec.features[first_index].name),
        ...
        for (index, behavior) in feature.behaviors.iter().enumerate() {
            if let Some(&first_index) = seen.get(behavior.name.as_str()) {
                result.add_error(SpecValidationError::DuplicateBehavior {
                    ...
                    behavior_b: format!(
                        "{}.behaviors[{}].{}",
                        feature.name, index, behavior.name
                    ),
    ...
    fn detect_circular_dependencies(&self, spec: &Spec, result: &mut ValidationResult) {
        ...
        if let Some(cycle_path) = graph.detect_cycles() {
            if cycle_path.len() >= 2 {
                result.add_error(SpecValidationError::CircularDependencyPath {
                    path: cycle_path,
                });
            }
        }
    ...
    pub fn sort_behaviors_by_priority(&self, spec: &Spec) -> Vec<(String, BehaviorPriority)> {
        ...
        let prioritized: Vec<(String, BehaviorPriority)> = spec
            .features
            .iter()
...
mod tests {
    ...
    fn test_validation_result_add_error() {
        let mut result = ValidationResult::new();
        result.add_error(SpecValidationError::MissingRequiredField("test".to_string()));
    ...
    fn test_validation_result_merge() {
        ...
        result2.add_error(SpecValidationError::MissingRequiredField("test".to_string()));
    ...
    fn test_dependency_graph_topological_sort() {
        ...
        let c_pos = order.iter().position(|n| n == "C").expect("C should be in order");
        let b_pos = order.iter().position(|n| n == "B").expect("B should be in order");
        let a_pos = order.iter().position(|n| n == "A").expect("A should be in order");
    ...
    fn test_validate_circular_feature_dependencies() {
        ...
        let has_cycle_error = result.errors.iter().any(|e| {
            matches!(e, SpecValidationError::CircularDependencyPath { .. })
        });
    ...
    fn test_feature_execution_order_valid() {
        ...
        let a_pos = order.iter().position(|n| n == "feature_a").expect("a in order");
        let b_pos = order.iter().position(|n| n == "feature_b").expect("b in order");

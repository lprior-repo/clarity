...
impl From<ArrayIndexError> for InterpolationError {
    fn from(err: ArrayIndexError) -> Self {
        match err {
            ...
            ArrayIndexError::IndexOutOfBounds { index, length } => {
                InterpolationError::IndexOutOfBounds {
...
impl Context {
    ...
    {
        Self {
            variables: vars.into_iter().map(|(k, v)| (k.into(), v.into())).collect(),
            request_body: None,
            response_body: None,
        }
    }
...
fn resolve_from_body(
    ...
    let body = body.ok_or_else(|| InterpolationError::VariableNotFound(format!("{body_name}.body")))?;
    ...
    if actual_path.is_empty() || actual_path == "body" {
        ...
        return serde_json::to_string(body)
            .map_err(|e| InterpolationError::JsonError(e.to_string()));
    }
...
mod tests {
    ...
    fn test_interpolate_string_with_array() -> Result<(), InterpolationError> {
        ...
        let result = interpolate_string("First: ${request.items[0]}, Last: ${request.items[-1]}", &ctx)?;
    ...
    fn test_error_display() {
        ...
        let err = InterpolationError::IndexOutOfBounds { index: 5, length: 3 };
        let msg = format!("{}", err);

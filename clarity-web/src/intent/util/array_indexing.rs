...
pub fn parse_path_component(component: &str) -> Result<(String, ArraySpec), ArrayIndexError> {
    ...
    match trimmed.find('[') {
        ...
        Some(open_pos) => {
            ...
            let close_pos = trimmed.find(']').ok_or_else(|| {
                ArrayIndexError::InvalidPath(format!("unclosed bracket in: {component}"))
            })?;
...
fn parse_index_spec(content: &str, original: &str) -> Result<ArraySpec, ArrayIndexError> {
    ...
    let index: usize = trimmed.parse().map_err(|_| {
        ArrayIndexError::InvalidPath(format!("invalid index in: {original}"))
    })?;
...
pub fn navigate_path(
    value: &Value,
    path: &[String],
) -> Result<Value, ArrayIndexError> {
    if path.is_empty() {
        return Ok(value.clone());
    }

    path.iter()
        .try_fold(value.clone(), |current, component| {
            let (field, spec) = parse_path_component(component)?;

            // Get the field value from the current object
            let field_value = current.get(&field).ok_or_else(|| {
                ArrayIndexError::FieldNotFound(field.clone())
            })?;
...
fn navigate_array(
    ...
    let array = value.as_array().ok_or_else(|| ArrayIndexError::NotAnArray {
...
mod tests {
    ...
    fn test_navigate_nested_field() {
        ...
        let result = navigate_path(&data, &["user".to_string(), "profile".to_string(), "name".to_string()]);
        assert_eq!(result, Ok(json!("Alice")));
    }

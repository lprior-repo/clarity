...
mod tests {
  ...
  fn test_resolve_dependencies_invalid_dependency() {
    ...
    let result = resolve_dependencies(&beads);
    assert!(matches!(result, Err(PlanError::InvalidDependency { .. })));
  }
  ...
  fn test_topological_sort_respects_phases() {
    ...
    let phase1_pos_a = sorted
      .iter()
      .position(|id| id == "phase1-a")
      .expect("found");
    let phase1_pos_b = sorted
      .iter()
      .position(|id| id == "phase1-b")
      .expect("found");
    let phase2_pos_a = sorted
      .iter()
      .position(|id| id == "phase2-a")
      .expect("found");
    let phase2_pos_b = sorted
      .iter()
      .position(|id| id == "phase2-b")
      .expect("found");
  ...
  fn test_validate_plan_dependencies_valid() {
    ...
    plan
      .add_bead(create_test_bead("a", 1, vec![]))
      .expect("add");
    plan
      .add_bead(create_test_bead("b", 1, vec!["a"]))
      .expect("add");
  ...
  fn test_validate_plan_dependencies_invalid() {
    let mut plan = ExecutionPlan::new("test".to_string());
    plan
      .add_bead(create_test_bead("a", 1, vec!["nonexistent"]))
      .expect("add");
  ...
  fn test_apply_resolution_to_plan_with_cycle() {
    let mut plan = ExecutionPlan::new("test".to_string());
    plan
      .add_bead(create_test_bead("a", 1, vec!["b"]))
      .expect("add");
    plan
      .add_bead(create_test_bead("b", 1, vec!["a"]))
      .expect("add");

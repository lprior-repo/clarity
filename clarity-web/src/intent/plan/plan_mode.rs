        ...
        .map(|phase| {
            ...
            if completed_phases.contains(&phase.phase_number) {
                ...
                if !prev_phase_complete {
                    ...
                    gated_phase
                        .blockers
                        .push(format!("Phase {} must be completed first", phase.phase_number - 1));
                }
            }
    ...
    for conflict in &session.conflicts {
        if conflict.chosen.is_none() {
            beads.push(PlanBead {
                ...
                title: format!("Resolve conflict: {} vs {}", conflict.between.0, conflict.between.1),
...
fn organize_into_phases(beads: &[PlanBead], session: &InterviewSession) -> Vec<Phase> {
    ...
    let phase_names = [
        (1, "Discovery", "Initial exploration and requirements gathering"),
...
fn assign_beads_to_phase(beads: &[PlanBead], phase_number: u32, _is_complete: bool) -> Vec<PlanBead> {
...
fn detect_blockers(session: &InterviewSession) -> Vec<String> {
    ...
    for gap in session.get_blocking_gaps() {
        blockers.push(format!("Unresolved gap: {} - {}", gap.field, gap.description));
    }

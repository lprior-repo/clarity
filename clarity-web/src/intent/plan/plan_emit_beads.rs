...
fn generate_beads_from_gaps(
  ...
  for gap in &session.gaps {
    ...
    let bead_result = PlanBead::new(format!("gap-{}", gap.id), title, gap.round);
...
fn group_answers_by_phase(session: &InterviewSession) -> Vec<(u32, Vec<&Answer>)> {
  let mut phase_map: std::collections::HashMap<u32, Vec<&Answer>> =
...
fn update_plan_phases(plan: &mut ExecutionPlan) {
  ...
  let mut phase_numbers: HashSet<u32> = plan.beads.iter().map(|b| b.phase).collect();
...
fn generate_api_beads(phase: u32) -> Vec<PlanBead> {
  ...
  tasks
    ...
    .filter_map(|(id, title, effort)| {
      PlanBead::new(format!("api-{id}"), title.to_string(), phase)
        .ok()
        .map(|b| {
...
fn generate_cli_beads(phase: u32) -> Vec<PlanBead> {
  ...
  tasks
    .iter()
    .filter_map(|(id, title, effort)| {
      PlanBead::new(format!("cli-{id}"), title.to_string(), phase)
        .ok()
        .map(|b| {
...
fn generate_event_beads(phase: u32) -> Vec<PlanBead> {
  ...
  tasks
    .iter()
    .filter_map(|(id, title, effort)| {
      PlanBead::new(format!("event-{id}"), title.to_string(), phase)
        .ok()
        .map(|b| {
...
fn generate_data_beads(phase: u32) -> Vec<PlanBead> {
  ...
  tasks
    .iter()
    .filter_map(|(id, title, effort)| {
      PlanBead::new(format!("data-{id}"), title.to_string(), phase)
        .ok()
        .map(|b| {
...
fn generate_workflow_beads(phase: u32) -> Vec<PlanBead> {
  ...
  tasks
    .iter()
    .filter_map(|(id, title, effort)| {
      PlanBead::new(format!("workflow-{id}"), title.to_string(), phase)
        .ok()
        .map(|b| {
...
fn generate_ui_beads(phase: u32) -> Vec<PlanBead> {
  ...
  tasks
    .iter()
    .filter_map(|(id, title, effort)| {
      PlanBead::new(format!("ui-{id}"), title.to_string(), phase)
        .ok()
        .map(|b| {
...
mod tests {
  ...
  fn test_emit_beads_with_gaps() {
    ...
    let gap_beads: Vec<&PlanBead> = beads
      .iter()
      .filter(|b| b.tags.contains(&"gap".to_string()))
      .collect();
  ...
  fn test_generate_profile_beads_all_profiles() {
    ...
    for profile in profiles {
      ...
      assert!(
        !beads.is_empty(),
        "Profile {profile:?} should generate beads"
      );
    }
  }
  ...
  fn test_update_plan_phases() {
    ...
    plan
      ...
      .expect("add");
    plan
      .add_bead(PlanBead::new("b2".to_string(), "Second".to_string(), 1).expect("valid"))
      .expect("add");
    plan
      .add_bead(PlanBead::new("b3".to_string(), "Third".to_string(), 2).expect("valid"))
      .expect("add");

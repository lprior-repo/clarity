...
impl ContextualError {
  ...
  pub fn location_string(&self) -> Option<String> {
    match (&self.source_file, &self.line, &self.column) {
      (Some(file), Some(line), Some(col)) => Some(format!("{file}:{line}:{col}")),
...
impl FieldFailure {
  ...
  pub fn invalid_type(
    ...
    Self::new(
      field,
      "invalid_type",
      format!("Expected {expected_str}, got {actual_str}"),
    )
...
impl fmt::Display for FieldFailure {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    if !self.suggestions.is_empty() {
      let suggestions_str = self.suggestions.iter().map(|s| s.text.as_str()).join(", ");
...
mod tests {
  ...
  fn levenshtein_unicode_support() {
    ...
    // Emoji
    assert_eq!(
      levenshtein("\u{2764}\u{fe0f}\u{200d}\u{1f525}", "\u{2764}\u{fe0f}"),
      2
    );
  }
  ...
  fn suggest_field_names_no_match() {
    let available = vec!["completely".to_string(), "different".to_string()];
  ...
  fn contextual_error_new() {
    let error = ContextualError::new(IntentError::MissingField("name".into()), "Name is required");
    assert!(error.is_ok());
  ...
  fn contextual_error_empty_message_rejected() {
    let error = ContextualError::new(IntentError::MissingField("name".into()), "");
    assert!(error.is_err());
  }
  ...
  fn contextual_error_with_suggestions() {
    let error = ContextualError::new(IntentError::UnknownField("nmae".into()), "Unknown field")
      .map_err(|_| ())
      .map_err(|()| "")
      .ok()
      .map(|e| e.with_suggestion(Suggestion::new("name", 2)));
  ...
  fn field_failure_unknown_with_suggestions() {
    let failure = FieldFailure::unknown_field(
      "nmae",
      vec![Suggestion::new("name", 2), Suggestion::new("mane", 2)],
    );
  ...
  fn format_error_with_suggestions() {
    let error = ContextualError::new(IntentError::UnknownField("nmae".into()), "Unknown field")
      ...
      .map_err(|()| "")
      .ok()
      .map(|e| e.with_suggestions(vec![Suggestion::new("name", 2)]));

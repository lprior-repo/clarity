...
impl QualityScore {
    ...
    pub fn gate_message(&self) -> String {
        if self.gate_passes() {
            ...
            format!("Quality gate: FAIL (need {QUALITY_GATE_THRESHOLD}, have {})", self.overall)
        }
    }
}
...
mod tests {
    ...
    fn test_quality_dimension_with_issues() {
        let dim = QualityDimension::new("Clarity", 60)
            .with_issues(vec![
    ...
    fn test_quality_score_gate_message() {
        ...
        assert_eq!(fail_exact.gate_message(), "Quality gate: FAIL (need 70, have 69)");
    }

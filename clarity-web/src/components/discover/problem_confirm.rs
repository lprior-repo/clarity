        ...
        move || {
            ...
            QualityScore::new(overall).with_dimensions(vec![
                ...
                QualityDimension::new("Realism", realism)
                    .with_issues(get_realism_issues(&response.points)),
...
mod tests {
    ...
    fn test_get_specificity_issues_short() {
        let points = vec!["Too short".to_string(), "Brief".to_string(), "Vague".to_string()];
        let issues = get_specificity_issues(&points);
    ...
    fn test_get_specificity_issues_good() {
        let points = vec![
            ...
            "Cost outweighs perceived benefits especially for smaller teams with limited budgets".to_string(),
        ];
        let issues = get_specificity_issues(&points);
        assert!(issues.is_empty());
    }

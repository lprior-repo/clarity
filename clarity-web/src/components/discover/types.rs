...
impl HoleType {
    ...
    pub const fn all() -> &'static [Self] {
        &[Self::DiscoveryHole, Self::EdgeCaseHole, Self::MotivationDropOff]
    }
...
mod tests {
    ...
    fn test_empty_string_treated_as_none() {
        let holes = HolePunchingResults::from_strings(
            "".to_string(),
            "   ".to_string(),
            "\t\n".to_string(),
        );
    ...
    fn test_address_method() {
        let holes = HolePunchingResults::new()
            ...
            .address(HoleType::EdgeCaseHole, "Handles offline gracefully".to_string())
            .address(HoleType::MotivationDropOff, "Progress indicator".to_string());
        assert!(holes.is_complete());
    ...
    fn test_address_method_empty_normalizes() {
        let holes = HolePunchingResults::new()
            .address(HoleType::DiscoveryHole, "".to_string());
        assert!(holes.discovery_hole.is_none());
    }
    ...
    fn test_explanation() {
        ...
        assert_eq!(holes.explanation(HoleType::DiscoveryHole), Some("Found via search"));

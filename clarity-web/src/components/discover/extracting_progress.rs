...
pub fn ExtractingProgress(props: ExtractingProgressProps) -> Element {
    ...
    let is_visible = matches!(status, ExtractionStatus::Extracting | ExtractionStatus::Complete);
...
mod tests {
    ...
    fn test_status_matches() {
        ...
        assert!(matches!(ExtractionStatus::Extracting, ExtractionStatus::Extracting));
        assert!(!matches!(ExtractionStatus::Idle, ExtractionStatus::Extracting));
    }

...
impl VorpFields {
  ...
  pub fn score(&self) -> u8 {
    ...
    let value_bonus = if self.value.split_whitespace().count() > 10 {
      10
    } else {
      0
    };
    let obvious_bonus = if self.obvious.split_whitespace().count() > 5 {
      10
    } else {
      0
    };
    let real_bonus = if self.real.split_whitespace().count() > 10 {
      10
    } else {
      0
    };
    let possible_bonus = if self.possible.split_whitespace().count() > 5 {
      10
    } else {
      0
    };
...
pub fn VorpInput(props: VorpInputProps) -> Element {
  ...
  let update_field = {
    ...
    move |field: &str, value: String| {
      ...
      let new_vorp = match field {
        "value" => VorpFields { value, ..current },
        "obvious" => VorpFields {
          obvious: value,
          ..current
        },
  ...
  let vorp_labels = [
    ...
    (
      "possible",
      "Possible",
      "Is this buildable with available resources?",
    ),
  ];
  ...
  let quality_score = use_memo({
    move || {
      ...
      QualityScore::new(overall).with_dimensions(vec![
        QualityDimension::new("Value", value_score).with_issues(get_field_issues(
          &fields.value,
          "Value",
          10,
        )),
        QualityDimension::new("Obvious", obvious_score).with_issues(get_field_issues(
          &fields.obvious,
          "Obvious",
          5,
        )),
        QualityDimension::new("Real", real_score).with_issues(get_field_issues(
          &fields.real,
          "Real",
          10,
        )),
        QualityDimension::new("Possible", possible_score).with_issues(get_field_issues(
          &fields.possible,
          "Possible",
          5,
        )),
...
fn get_field_issues(text: &str, field_name: &str, min_words: usize) -> Vec<String> {
  ...
  if word_count < min_words {
    return vec![format!(
      "{field_name} needs more detail (at least {min_words} words)"
    )];
  }
...
mod tests {
  ...
  fn test_calculate_field_score_long() {
    let score = calculate_field_score(
      "This is a very long text with many more words than the minimum requirement",
      5,
    );
    assert_eq!(score, 90);
  }
  ...
  fn test_get_field_issues_good() {
    let issues = get_field_issues(
      "This is a good long enough description with many words",
      "Value",
      10,
    );

...
fn cmds_for_step(id: &str, val: &str) -> Vec<TermCmd> {
    ...
    match id {
        ...
        "persona" => vec![TermCmd {
            agent: "planner".into(),
            cmd: format!("bd create -t task --parent bd-b2e1 --title \"Persona: {}...\"", v),
            out: "Created bd-b2e1.1  [task] Persona".into(),
        }],
        "scenario" => vec![
            TermCmd {
                agent: "planner".into(),
                cmd: "bd create -t task --parent bd-b2e1 --title \"North Star Scenario\""
                    .into(),
...
pub fn PlanningCoach(
    ...
    let completed_ids: Vec<String> = answers
        .read()
        .iter()
    ...
    let current_step = phase_steps
        .iter()
        .find(|s| !completed_ids.contains(&s.id));

...
fn group_color(group: &str) -> (String, String) {
  match group {
    "thesis" => (
      "hsl(221, 83%, 53%)".into(),
      "hsl(221, 83%, 53%, 0.15)".into(),
    ),
    "persona" => (
      "hsl(262, 83%, 58%)".into(),
      "hsl(262, 83%, 58%, 0.15)".into(),
    ),
    "scenario" => (
      "hsl(142, 71%, 45%)".into(),
      "hsl(142, 71%, 45%, 0.15)".into(),
    ),
    "usecase" => ("hsl(38, 92%, 50%)".into(), "hsl(38, 92%, 50%, 0.15)".into()),
    "task" => ("hsl(0, 72%, 51%)".into(), "hsl(0, 72%, 51%, 0.15)".into()),
    "effect-root" => (
      "hsl(173, 80%, 40%)".into(),
      "hsl(173, 80%, 40%, 0.15)".into(),
    ),
    "effect-leaf" => (
      "hsl(280, 70%, 55%)".into(),
      "hsl(280, 70%, 55%, 0.15)".into(),
    ),
    "effect" => (
      "hsl(200, 70%, 50%)".into(),
      "hsl(200, 70%, 50%, 0.15)".into(),
    ),
...
pub fn GraphVisualizer(answers: Signal<Vec<Answer>>) -> Element {
  ...
  let effects_output = trace_effects(
    &answers_guard
      .iter()
      .filter(|a| {
        let step = a.step_id.to_lowercase();
  ...
  let legend_items = if use_effects {
    vec![
      LegendItem {
        group: "Causal Root".to_string(),
        color: group_color("effect-root").0,
      },
      LegendItem {
        group: "Causal Link".to_string(),
        color: group_color("effect").0,
      },
      LegendItem {
        group: "Outcome".to_string(),
        color: group_color("effect-leaf").0,
      },
    ]
  } else {
    build_legend_items()
